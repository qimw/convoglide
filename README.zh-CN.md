...
- 原始 ChatGPT 页面大约在 `12,422 ms` 才第一次显示真实会话标题，然后大约到 `18,452 ms` 左右探针就开始超时
- 在一轮干净对照里，ConvoGlide 新默认档位（`keep 20`）的主会话响应大约在 `10,746 ms` 返回，真实会话标题大约在 `14,003 ms` 出现，并且能稳定撑过 `50,000 ms`
- 也就是说，真正能归功于前端优化的那段“主响应回来后到页面真正显示出来”的浏览器处理时间，从约 `4,815 ms` 缩短到了约 `3,257 ms`，而且页面不会像原始版那样很快变成半卡死状态
- 在同样的干净对照里，原始页面连一次合成长滚动探测都撑不完，而 ConvoGlide 的 `20` 档和 `80` 档都测出了 `smooth`
