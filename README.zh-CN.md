...
当前公开 alpha 默认保留最近 `8` 条消息，因为这是目前第一档把这条 benchmark 对话压进 `15 s` 以内、同时还保住顺滑长滚动结果的配置。

现在 runtime 里还加入了一个更贴近架构目标的冷启动 bootstrap 路径：在本地还没有会话缓存的首次打开时，它可以先把更小的 bootstrap 窗口交给当前页面，同时把正常 `keep 8` 的裁剪结果写进本地缓存，供下一次重开复用。这仍然不是服务端分页，但已经更接近“先给用户最小可用切片”的思路。

最新的探索样本已经证明这条路不是纸上谈兵：在当前 benchmark 线程上，冷启动 `bootstrap = 2` 的单次样本打到了 `13.434 s`，同时长滚动依然是 `smooth`。这还不是正式公开口径，但已经说明 bootstrap 窗口现在是一个真实可调的首屏优化旋钮。

如果你想保留更多近期历史，也可以手动切到 `80` 或 `120`。
...
新的架构文档已经把“插件能做什么、不能做什么”写清楚了。ConvoGlide 的定位是浏览器侧的渐进式渲染层，而不是服务端分页系统。
